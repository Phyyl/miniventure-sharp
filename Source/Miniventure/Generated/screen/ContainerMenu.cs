...
public class ContainerMenu : Menu {
	...
	public override void tick() {
		...
		if (input.left.clicked) { //if the left key is params pressed[]
		...
		if (input.right.clicked) { //if the right key is params pressed[]
		...
		if (input.attack.clicked && len > 0) { // If the "Attack" key is pressed and the inventory's size is bigger than params 0[]
