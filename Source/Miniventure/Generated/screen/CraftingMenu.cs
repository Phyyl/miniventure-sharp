...
public class CraftingMenu : Menu {
	...
	public CraftingMenu(List<Recipe> recipes, Player player) {
		this.recipes = new List<Recipe>(recipes); // Assigns the recipes
	...
	public override void tick() {
		...
		if (input.attack.clicked && len > 0) {// If the "Attack" key is pressed and the recipe's size is bigger than params 0[]
			...
			if (r.canCraft) { // If the player can craft the params item[]
	...
	public override void render(Screen screen) {
		...
		if (recipes.size() > 0) { // If the size of the recipe list is above params 0[]
