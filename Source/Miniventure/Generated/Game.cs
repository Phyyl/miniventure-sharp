...
public class Game : Canvas, Runnable {
	private static readonly long serialVersionUID = 1L;
	
	// This is the name on the application window
	public static readonly string NAME = "Miniventure";
	// This is the hight of the game * scale
	public static readonly int HEIGHT = 200;
	// This is the width of the game * scale
	public static readonly int WIDTH = 267;
	private static readonly int SCALE = 3; // scales the window
	...
	private Buffer pixels; // the array of pixels that will be displayed on the screen.
	private bool running = false; // This stores if the game is running or paused
	...
	public bool hasWon = false; 	// If the player wins this is set to true
	...
	public virtual void setMenu(Menu menu) {
		this.menu = menu;
	...
	private void init() {
		...
		try {
			screen = new Screen(WIDTH, HEIGHT, new SpriteSheet(ImageIO.read(typeof(Game).getResourceAsStream("/icons.png"))));
			lightScreen = new Screen(WIDTH, HEIGHT, new SpriteSheet(ImageIO.read(typeof(Game).getResourceAsStream("/icons.png"))));
		} catch (IOException e) {
			e.printStackTrace();
		}

		resetGame(); // starts a new params game[] for some reason?
	...
	public virtual void run() {
		long lastTime = SystemInfo.nanoTime();
		double unprocessed = 0;
		// Nanoseconds per Tick
		double nsPerTick = 1000000000.0 / 60; // There are 60 ticks per second.
		int frames = 0;
		int ticks = 0;
		long lastTimer1 = SystemInfo.currentTimeMillis(); // current time in milliseconds.

		init(); // preps the game by setting up the screens and colors.

		while (running) {
			long now = SystemInfo.nanoTime();
			...
			while (unprocessed >= 1) { // If there is unprocessed time, then tick.
				...
				shouldRender = true; // causes the should render to be params true[] why is this here since it was already true? whatever.
			}
			...
			if (SystemInfo.currentTimeMillis() - lastTimer1 > 1000) { //updates every 1 second
				lastTimer1 += 1000;//adds a second to the timer
				Console.WriteLine(ticks + " ticks, " + frames + " fps"); // prints out the number of ticks, and the amount of frames to the console.
	...
	public virtual void scheduleLevelChange(int dir) {
	...
	public static void main(string[] args) {
		Game game = new GameOverrides(); // creates a new game.
