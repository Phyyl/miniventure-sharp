...
public class HoleTile : Tile {
	...
	public override void render(Screen screen, Level level, int x, int y) {
		...
		bool u = !level.getTile(x, y - 1).connectsToLiquid(); // Checks if the tile above can NOT connect to a liquid
		bool d = !level.getTile(x, y + 1).connectsToLiquid(); // Checks if the tile above can NOT connect to a liquid
		bool l = !level.getTile(x - 1, y).connectsToLiquid(); // Checks if the tile above can NOT connect to a liquid
		bool r = !level.getTile(x + 1, y).connectsToLiquid(); // Checks if the tile above can NOT connect to a liquid
		...
		if (!u && !l) { // if the tile to the left, and the tile above can connect to params liquids[]
		...
		if (!u && !r) { // if the tile to the right, and the tile above can connect to params liquids[]
		...
		if (!d && !l) { // if the tile to the left, and the tile below can connect to params liquids[]
		...
		if (!d && !r) { // if the tile to the right, and the tile below can connect to params liquids[]
