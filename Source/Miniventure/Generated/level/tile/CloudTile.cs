...
public class CloudTile : Tile {
	public CloudTile(int id) : base(id) {//assigns the id
	}
	...
	/* Commenter Note: All sentences with a "*" at the end means I'm making a guess, and not 100% sure. Please confirm it sometime in the future. */
		...
		if (!u && !l) { // If there is no infiniteFall tile above or to the left of params this[]
		...
		if (!u && !r) { // If there is no infiniteFall tile above or to the right of params this[]
		...
		if (!d && !l) { // If there is no infiniteFall tile below or to the left of params this[]
		...
		if (!d && !r) { // If there is no infiniteFall tile below or to the right of params this[]
