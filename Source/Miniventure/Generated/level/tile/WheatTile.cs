...
public class WheatTile : Tile {
	public WheatTile(int id) : base(id) { // assigns the id
	}
	...
	public override void render(Screen screen, Level level, int x, int y) {
		...
		if (icon >= 3) { // if the icon is larger or equal to 3.
			...
			if (age == 50) { // if the age is equal to 50 params then[]
	...
	public override void tick(Level level, int xt, int yt) {
		...
		if (random.nextBoolean() == false) return; // if the random bool is false, then skip the rest of the code
	...
	private void harvest(Level level, int x, int y) {
		...
		if (age == 50) { // if the age is equal to 50 (fully grown) params then[]
			count = random.nextInt(3) + 2; // count will be anywhere between 2 to 4
		} else if (age >= 40) { // if the age is larger or equal to 40 params then[]
