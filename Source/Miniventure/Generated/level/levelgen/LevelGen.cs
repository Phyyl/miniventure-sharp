...
public class LevelGen {
	...
	public static byte[][] createAndValidateTopMap(int w, int h) {
		do { // Keep repeating this loop until it's params done[]
	...
	private static byte[][] createTopMap(int w, int h) {
		...
		for (int y = 0; y < h; y++) { // Loops through the height of the map
			for (int x = 0; x < w; x++) { // A loop inside a loop that loops through the width of the map.
				...
				if (val < -0.5) { // If the readonly value is less than -0.params 5[]
					map[i] = Tile.water.id; // the tile will become water
				} else if (val > 0.5 && mval < -1.5) { // else if the val is larger 0.5 and mval is smaller than -1.params 5[]
					map[i] = Tile.rock.id; // the tile will become rock
				} else { // If the values don't agree with those params then[]
				...
				for (int j = 0; j < 100; j++) { // A loop inside a loop inside the main loop, repeats 100 times.
					...
					for (int yy = yo - 1; yy <= yo + 1; yy++) // Loopception, Loops if yy is smaller or equal to yo + 1
						for (int xx = xo - 1; xx <= xo + 1; xx++) // Loopception, Loops if xx is smaller or equal to xo + 1 
							if (xx >= 0 && yy >= 0 && xx < w && yy < h) { // if xx or yy is equal or larger than 0, and smaller than the width and height of the params map[]
								if (map[xx + yy * w] == Tile.grass.id) { // If the specific xx and yy coordinates happen to be a grass params tile[]
		...
		for (int i = 0; i < w * h / 100; i++) {
			...
			if (xx >= 0 && yy >= 0 && xx < w && yy < h) { // if xx or yy is equal or larger than 0, and smaller than the width and height of the params map[]
				if (map[xx + yy * w] == Tile.sand.id) { // If the specific xx and yy coordinates happen to be a sand params tile[]
		...
		for (int i = 0; i < w * h / 100; i++) {
			...
			for (int yy = y - 1; yy <= y + 1; yy++) // Loops if yy is smaller or equal to y + 1
				for (int xx = x - 1; xx <= x + 1; xx++) { // Loops if xx is smaller or equal to x + 1
					if (map[xx + yy * w] != Tile.rock.id) goto stairsLoop; // If the current tile is NOT a rock tile, then it skips the loops back to the top.
				}
	...
	private static byte[][] createUndergroundMap(int w, int h, int depth) {
		...
		for (int y = 0; y < h; y++) { // Loops through the height of the map
			for (int x = 0; x < w; x++) { // Loops through the width of the map
				...
				} else if (val > -2 && (mval < -1.7 || nval < -1.4)) {//if val is larger than -2, and mval is smaller than -1.7 OR nval is smaller than -1.4 params then[]
...

		{
			...
			for (int i = 0; i < w * h / 400; i++) {
				...
				for (int j = 0; j < 30; j++) { // A loop that occurs 30 times
					...
					if (xx >= r && yy >= r && xx < w - r && yy < h - r) { // If xx & yy are equal to or larger than r, and smaller than (w - r) and (h - r) params then[]
						if (map[xx + yy * w] == Tile.rock.id) { // If the current tile is a rock params tile[]
		...
		if (depth < 3) { // If the depth is smaller than 3 params then[]
			...
			for (int i = 0; i < w * h / 100; i++) {
				...
				for (int yy = y - 1; yy <= y + 1; yy++)  // Loops if yy is smaller or equal to y + 1
					for (int xx = x - 1; xx <= x + 1; xx++) {  // Loops if xx is smaller or equal to x + 1
						if (map[xx + yy * w] != Tile.rock.id) goto stairsLoop; // If the current Tile is not a rock then start the main loop all over.
					}
	...
	private static byte[][] createSkyMap(int w, int h) {
		...
		for (int y = 0; y < h; y++) { // loops through the height of the map
			for (int x = 0; x < w; x++) { // loops though the width of the map
				...
				if (val < -0.25) { // If val is smaller than -0.25 params then[]
		...
		for (int i = 0; i < w * h / 50; i++) { //loops through all the numbers that are less than (width * height / 50)
			...
			for (int yy = y - 1; yy <= y + 1; yy++) // Loops if yy is smaller or equal to y + 1
				for (int xx = x - 1; xx <= x + 1; xx++) { // Loops if xx is smaller or equal to x + 1
					if (map[xx + yy * w] != Tile.cloud.id) goto cactusLoop; // If the current tile is NOT a cloud tile, then it skips the loops back to the top.
				}
		...
		for (int i = 0; i < w * h; i++) { //loops through the entire map
			...
			for (int yy = y - 1; yy <= y + 1; yy++) // Loops if yy is smaller or equal to y + 1
				for (int xx = x - 1; xx <= x + 1; xx++) { // Loops if xx is smaller or equal to x + 1
					if (map[xx + yy * w] != Tile.cloud.id) goto stairsLoop; // If the current tile is NOT a cloud tile, then it skips the loops back to the top.
				}
