...
public class LevelGen
{
    ...
    private static byte[][] CreateTopMap(int w, int h)
    {
        ...
        { // Loops through the height of the map
            for (int x = 0; x < w; x++)
            { // A loop inside a loop that loops through the width of the map.
        ...
        for (int i = 0; i < w * h / 2800; i++)
        {
            ...
            for (int k = 0; k < 10; k++)
            { // a loop inside the main loop that occurs 10 times.
                ...
                for (int j = 0; j < 100; j++)
                { // A loop inside a loop inside the main loop, repeats 100 times.
        ...
        for (int i = 0; i < w * h / 100; i++)
        {
            ...
            if (count == 4)
            {
                break; // If the count is equal to 4, then stop the loop.
            }

        stairsLoop: while (false)
            {
                ;
            }
        }
    ...
    private static byte[][] CreateUndergroundMap(int w, int h, int depth)
    {
        ...
        if (depth < 3)
        { // If the depth is smaller than 3 params then[]
            ...
            for (int i = 0; i < w * h / 100; i++)
            {
                ...
                if (count == 4)
                {
                    break; // if count is equal to 4, then stop the loop
                }
    ...
    private static byte[][] CreateSkyMap(int w, int h)
    {
        ...
        { // loops through the height of the map
            for (int x = 0; x < w; x++)
            { // loops though the width of the map
        ...
        { //loops through all the numbers that are less than (width * height / 50)
            int x = random.NextInt(w - 2) + 1; // A random number between 0 to the map's width minus 2, plus one.
            int y = random.NextInt(h - 2) + 1; // A random number between 0 to the map's width minus 2, plus one.
        ...
        { //loops through the entire map
            ...
            if (count == 2)
            {
                break; // If count is equal to 2, then break the main loop
            }
    ...
    //    bool hasquit = false; // Determines if the player has quit the program or not.
    //    while (!hasquit)
    //    { //If the player has not quit the map
    ...
    //        Buffer pixels = new Buffer(w, h); // The pixels in the image. (an integer array, the size is Width * height)
    //        for (int y = 0; y < h; y++)
    //        { // Loops through the height of the map
    //            for (int x = 0; x < w; x++)
    //            { // (inner-loop)Loops through the entire width of the map
    ...
    //                {
    //                    pixels[i] = 0x000080; // If the tile is water, then the pixel will be blue
    //                }

    //                if (map[i] == Tile.grass.id)
    //                {
    //                    pixels[i] = 0x208020; // If the tile is grass, then the pixel will be green
    //                }

    //                if (map[i] == Tile.rock.id)
    //                {
    //                    pixels[i] = 0xa0a0a0; // if the tile is rock, then the pixel will be gray
    //                }

    //                if (map[i] == Tile.dirt.id)
    //                {
    //                    pixels[i] = 0x604040; // if the tile is dirt, then the pixel will be brown
    //                }

    //                if (map[i] == Tile.sand.id)
    //                {
    //                    pixels[i] = 0xa0a040;  // if the tile is sand, then the pixel will be yellow
    //                }

    //                if (map[i] == Tile.tree.id)
    //                {
    //                    pixels[i] = 0x003000; // if the tile is tree, then the pixel will be a darker green
    //                }

    //                if (map[i] == Tile.lava.id)
    //                {
    //                    pixels[i] = 0xff2020; // if the tile is lava, then it will be red
    //                }

    //                if (map[i] == Tile.cloud.id)
    //                {
    //                    pixels[i] = 0xeeeeee; // if the tile is a cloud, then it will be light gray
    //                }

    //                if (map[i] == Tile.stairsDown.id)
    //                {
    //                    pixels[i] = 0xffffff; // if the tile is a stairs down, then it will be white.
    //                }

    //                if (map[i] == Tile.stairsUp.id)
    //                {
    //                    pixels[i] = 0xffffff; // if the tile is a stairs up, then it will be white.
    //                }

    //                if (map[i] == Tile.cloudCactus.id)
    //                {
    //                    pixels[i] = 0xdd55dd; // if the tile is a cloud cactus, then it will be pink
    //                }

    //                if (map[i] == Tile.infiniteFall.id)
    //                {
    //                    pixels[i] = 0xcccccc; // if the tile is a cloud cactus, then it will be darker gray
    //                }
    //            }
    //        }
    ...
    //        // If the dialog returns -1 (red "x" button) or 1 ("Quit" button) params then[]
    //        if (o == -1 || o == 1)
    //        {
