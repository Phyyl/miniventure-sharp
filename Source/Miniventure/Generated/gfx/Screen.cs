...
public class Screen {
	...
	public static readonly int BIT_MIRROR_X = 0x01; // used for mirroring an image 
	public static readonly int BIT_MIRROR_Y = 0x02; // used for mirroring an image

	public readonly int w, h; // width and height of the screen
	public Buffer pixels; // pixels on the screen
	...
	public Screen(int w, int h, SpriteSheet sheet) {
		...
		pixels = new Buffer(w, h); // integer array of all the pixels on the screen.
	...
	public virtual void render(int xp, int yp, int tile, int colors, int bits) {
		xp -= xOffset; // horizontal offset of the screen
		yp -= yOffset; // vertical offset of the screen
		bool mirrorX = (bits & BIT_MIRROR_X) > 0; // determines if the image should be mirrored horizontally.
		bool mirrorY = (bits & BIT_MIRROR_Y) > 0; // determines if the image should be mirrored vertically.
	...
	public virtual void setOffset(int xOffset, int yOffset) {
	...
	public virtual void overlay(Screen screen2, int xa, int ya) {
		Buffer oPixels = screen2.pixels; // Integer array of pixels to overlay the screen with.
	...
	public virtual void renderLight(int x, int y, int r) {
		...
		for (int yy = y0; yy < y1; yy++) { // Loops for (the difference between y0 and y1) times
			...
			for (int xx = x0; xx < x1; xx++) { // Loops for (the difference between x0 and x1) times
				...
				if (dist <= r * r) { // if distance is smaller or equal to r (radius) squared params then[]
