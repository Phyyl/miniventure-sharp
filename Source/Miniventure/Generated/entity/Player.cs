...
public class Player : Mob {
	...
	public override void tick() {
		base.tick(); // ticks the parent (Mob.java)
		...
		if (onTile == Tile.stairsDown || onTile == Tile.stairsUp) { // if the tile is a stairs up or stairs params down[]
			if (onStairDelay == 0) { // if the stair delay is 0 params then[]
		...
		/* if stamina is smaller or equal to 0, and if the recharge & recharge delay are both 0 params then[]  */
		if (stamina <= 0 && staminaRechargeDelay == 0 && staminaRecharge == 0) {
			staminaRechargeDelay = 40; // the recharge delay will equal 40 
		}

		if (staminaRechargeDelay > 0) { // if the recharge delay is above 0 params then[]
			staminaRechargeDelay--; // minus the recharge delay by 1.
		}

		if (staminaRechargeDelay == 0) { // if the stamina recharge delay is 0 params then[]
			staminaRecharge++; // the stamina recharge adds up.
			if (isSwimming()) { // if the player is swimming params then[]
				staminaRecharge = 0; // the recharge is 0.
			}
			while (staminaRecharge > 10) { // while the stamina recharge is above 10 params then[]
		...
		if (isSwimming() && tickTime % 60 == 0) { // if the player is swimming and the remainder of (tickTime/60) equals 0 params then[]
			if (stamina > 0) { // if stamina is above 0 params then[]
		...
		if (staminaRechargeDelay % 2 == 0) { // if the remainder of (staminaRechargeDelay/2) equals 0 params then[]
			move(xa, ya); // move the player in the x & y acceleration
		}

		if (input.attack.clicked) { // if the player presses the attack params button[]
			if (stamina == 0) { // if the player's stamina is params 0[]
				// nothing
			} else { // if the player's stamina is larger than 0 params then[]
		...
		if (input.menu.clicked) { // if the player presses the menu params button[]
	...
	private void attack() {
		...
		if (activeItem != null) { // if the player has a active Item
			...
			if (xt >= 0 && yt >= 0 && xt < level.w && yt < level.h) { // if (xt & yt) are larger or equal to 0 and less than the level's width and params height[]
				if (activeItem.interactOn(level.getTile(xt, yt), level, xt, yt, this, attackDir)) { // if the interactOn() method in an item returns params true[]
					done = true; // done equals true
				} else {
					if (level.getTile(xt, yt).interact(level, xt, yt, this, activeItem, attackDir)) { // if the interact() method in an item returns params true[]
						done = true; // done equals true
					}
				}
				if (activeItem.isDepleted()) { // if the activeItem has 0 resources left params then[]
		...
		if (activeItem == null || activeItem.canAttack()) { // if there is no active item, OR if the item can be used to params attack[]
	...
	public override void render(Screen screen) {
		...
		if (isSwimming()) { // if the player is params swimming[]
			yo += 4; // y offset is moved up by 4
			int waterColor = Color.get(-1, -1, 115, 335) ; // color of water circle
			if (tickTime / 8 % 2 == 0) { // if the remainder of (tickTime/8)/2 is equal to params 0[]
		...
		if (hurtTime > 0) { // if the player is params hurt[]
			col = Color.get(-1, 555, 555, 555); // then the color is white
		}

		if (activeItem is FurnitureItem) { // if the active item is a furniture item
	...
	public override bool findStartPos(Level level) {
		while (true) { // will loop until it returns
			...
			if (level.getTile(x, y) == Tile.grass) { // if the tile at the x & y coordinates is a grass tile params then[]
	...
	public override int getLightRadius() {
		...
		if (activeItem != null) { // if the player has an item
			if (activeItem is FurnitureItem) { // if item is a furniture item
	...
	public virtual void die() {
		base.die(); // calls the die() method in Mob.java
	...
	public virtual void touchedBy(Entity entity) {
		if (!(entity is Player)) { // if the entity is not a player.
