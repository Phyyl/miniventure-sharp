...
public class AirWizard : Mob {
	...
	public override void tick() {
		base.tick(); // ticks the Entity.java part of this class

		if (attackDelay > 0) { // if the attackDelay is larger than params 0[]
		...
		if (level.player != null && randomWalkTime == 0) { // if there is a player around, and the randomWalkTime is equal to 0
			...
			if (xd * xd + yd * yd < 32 * 32) { // if the x-distance� + y-distance� is smaller than 32� params then[]
			...
			} else if (xd * xd + yd * yd > 80 * 80) { // if the x-distance� + y-distance� is smaller than 80� params then[]
		...
		if (!move(xa * speed, ya * speed) || random.nextInt(100) == 0) { // If the air wizard is not moving & a random value (0 to 99) equals params 0[]
		...
		if (randomWalkTime > 0) { // if randomWalkTime is larger than params 0[]
			randomWalkTime--; // decrease randomWalkTime by 1
			if (level.player != null && randomWalkTime == 0) { // if there is a player around, and the randomWalkTime is equal to 0
				int xd = level.player.x - x; // the horizontal distance between the player and the air wizard.
				int yd = level.player.y - y; // the vertical distance between the player and the air wizard.
				
				/* if a random number (0 to 3) equals 0 and the x-distance� + y-distance� is smaller than 50� params then[]*/
				if (random.nextInt(4) == 0 && xd * xd + yd * yd < 50 * 50) {
					if (attackDelay == 0 && attackTime == 0) { // if attackDelay & attackTime equal 0, params then[]
	...
	public override void render(Screen screen) {
		...
		if (health < 200) { // if health is above 200
			if (tickTime / 3 % 2 == 0) { // if the remainder of ((tickTime/3)/2) is equal to params 0[]
			...
			if (tickTime / 5 % 4 == 0) { // if the remainder of ((tickTime/5)/4) is equal to params 0[]
				col1 = Color.get(-1, 500, 100, 555); // change colors
				col2 = Color.get(-1, 500, 100, 532); // change colors
			}
		}
		if (hurtTime > 0) {//if the air wizards hurt time is above params 0[] (hurtTime value in Mob.java)
	...
	public virtual void touchedBy(Entity entity) {
		if (entity is Player) { // if the entity happens to be the params player[]
