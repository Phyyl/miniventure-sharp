...
public class GrassTile : Tile {
	public GrassTile(int id) : base(id) { // assigns the id
	...
	public override void render(Screen screen, Level level, int x, int y) {
		...
		if (!u && !l) { // if the tile above and the tile to the left can connect to grass params then[]
		...
		if (!u && !r) {  // if the tile above and the tile to the right can connect to grass params then[]
		...
		if (!d && !l) {  // if the tile below and the tile to the left can connect to grass params then[]
		...
		if (!d && !r) {  // if the tile below and the tile to the right can connect to grass params then[]
	...
	public override void tick(Level level, int xt, int yt) {
		...
		if (level.getTile(xn, yn) == Tile.dirt) { // if the next positions are a dirt tile params then[]
		...
		if (item is ToolItem) { // if the item happens to be a tool
			...
			if (tool.type == ToolType.shovel) { // if the type of tool is a params shovel[]
				if (player.payStamina(4 - tool.level)) { // if the player can pay the params stamina[]
					...
					if (random.nextInt(5) == 0) { // if a random value between 0 to 4 equals 0 params then[]
			...
			if (tool.type == ToolType.hoe) { // if the type of tool is a params hoe[]
				if (player.payStamina(4 - tool.level)) { // if the player can pay the params stamina[]
					Sound.monsterHurt.play(); // plays a sound
					if (random.nextInt(5) == 0) { // if a random value between 0 to 4 equals 0 params then[]
						//Adds seeds to the world
