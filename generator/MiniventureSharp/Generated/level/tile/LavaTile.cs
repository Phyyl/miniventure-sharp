...
public class LavaTile : Tile {
	public LavaTile(int id) : base(id) { // assigns the id
	...
	public override void render(Screen screen, Level level, int x, int y) {
		...
		bool u = !level.getTile(x, y - 1).connectsToLava; // Checks if the tile above can NOT connect to lava
		bool d = !level.getTile(x, y + 1).connectsToLava; // Checks if the tile below can NOT connect to lava
		bool l = !level.getTile(x - 1, y).connectsToLava; // Checks if the tile to the left can NOT connect to lava
		bool r = !level.getTile(x + 1, y).connectsToLava; // Checks if the tile to the right can NOT connect to lava
		...
		if (!u && !l) { // if the tile to the left, and the tile above can connect to params lava[]
		...
		if (!u && !r) { // if the tile to the right, and the tile above can connect to params lava[]
		...
		if (!d && !l) { // if the tile to the left, and the tile below can connect to params lava[]
		...
		if (!d && !r) { // if the tile to the right, and the tile below can connect to params lava[]
	...
	public override void tick(Level level, int xt, int yt) {
		...
		if (level.getTile(xn, yn) == Tile.hole) { // if the next positions are a hole tile params then[]
