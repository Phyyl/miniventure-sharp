...
public class Spark : Entity {
	...
	public override void tick() {
		...
		if (time >= lifeTime) { // if time is larger or equal to lifeTime params then[]
		...
		for (int i = 0; i < toHit.size(); i++) { // cycles through the list
			...
			if (e is Mob && !(e is AirWizard)) { // if the entity is a mob, but not a Air Wizard params then[]
	...
	public override void render(Screen screen) {
		...
		if (time >= lifeTime - 6 * 20) {// if time is larger or equal to lifeTime - 6 * 20 params then[]
