...
public class Entity {
	public readonly Random random = new Random(); // Used for random numbers, example: random.randInt(5)
	...
	public bool removed; // Determines if the entity should be removed from the level
	...
	public virtual bool intersects(int x0, int y0, int x1, int y1) {
		/* if (x position + horizontal radius) is NOT smaller than x0 params OR[] 
		 * if (y position + vertical radius) is NOT smaller than y0 params OR[] 
		 * if (x position - horizontal radius) is NOT larger than x1 params OR[] 
	...
	public virtual bool move(int xa, int ya) {
		if (xa != 0 || ya != 0) { // If the horizontal acceleration OR vertical acceleration does NOT equal 0 params then[]
			bool stopped = true; // stopped value, used for checking if the entity has stopped.
			...
			if (!stopped) { // if stopped equals false params then[]
	...
	public virtual bool move2(int xa, int ya) {
		...
		if (xa != 0 && ya != 0) throw new ArgumentException("Move2 can only move along one axis at a time!");
		...
		for (int yt = yt0; yt <= yt1; yt++) // cycles through yt0 to yt1
			for (int xt = xt0; xt <= xt1; xt++) { // cycles through xt0 to xt1
				/* params If[]
				 * xt is larger or equal to xto0 params AND[]
				 * xt is smaller or equal to xto1 params AND[]
				 * yt is larger or equal to yto0 params AND[]
				...
				if (!level.getTile(xt, yt).mayPass(level, xt, yt, this)) { // If the entity cannot pass this params block[]
		...
		for (int i = 0; i < isInside.size(); i++) { // loops through isInside list
			...
			if (e.blocks(this)) { // if the entity can block this entity params then[]
				return false; // return false
			}
		}
